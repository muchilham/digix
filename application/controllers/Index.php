...
class Index extends CI_Controller {
	...
	public function index()
	{
		...
		$data['select_testimoni'] = $this->Data_model->select_testimoni();
	...
	public function register()
	{
		...
		if($this->Data_model->insert_account($data))
		{
			...
			if($this->email->send()) {
				$this->session->set_flashdata('notif', '<div id="register-success"></div>');
				redirect('?stat=register-success');
			  }
		}
		else
		{
			$this->session->set_flashdata('notif', '<div id="register-failed"></div>');
			redirect('?stat=register-failed');
		}
	}
