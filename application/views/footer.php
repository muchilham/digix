...
<?php if(!$this->session->logged_in)
    ...
    <div id="login-modal" class="modal">
        ...
        <form id="form-login" class="col s12" action="<?php echo base_url();?>index/login" method="POST">
            ...
            <div class="row">
                <div class="col s12">
                    <a href="<?php echo base_url(); ?>register" > Belum Punya Akun? Daftar di sini </a>
                </div> 
            </div>
...
<script type="text/javascript">
      // $("#pg-form").clientSidePagination();

      <?php 
        if(isset($_GET["stat"]))
        {
          if($_GET["stat"] == 'register-success')
          {
      ?>
              $(document).ready(function() {
                M.toast({html: 'Register success. Check your email!', classes: 'green'})  
              });
      <?php } 
            else if ($_GET["stat"] == 'register-failed')
            {
      ?>
              $(document).ready(function() {
                M.toast({html: 'Register Failed. Try Again!', classes: 'red'})  
              });
      <?php
            }
        }    
